...
async def scrape_zillow_property(url: str):
    """
    Attempt to scrape some property information from a Zillow page
    using a basic HTTP GET request and BeautifulSoup.
    """
    headers = {
        ...
        "if-none-match": 'W/"bfa8d-dcrFqqKQV+vIZisrs5DTeKliHZE"',
    ...
    for script_tag in soup.find_all("script"):
        raw_text = script_tag.text.strip()
        if "zpid" in raw_text or "address" in raw_text:
            try:
                start_index = raw_text.find('{')
                end_index = raw_text.rfind('}') + 1
                if start_index != -1 and end_index != -1:
                    possible_json_string = raw_text[start_index:end_index]
...
async def main():
    ...
    scraped_data = scrape_zillow_property(zillow_url)
    ...
    data = await extract_data("scraped_data.json")
    print(data[0])
